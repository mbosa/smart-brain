...
class App extends Component {
  ...
  calculateFaceLocation = (data) => {
    ...
    for (let i = 0; i < regions.length; i++) {
      const oneFace = regions[i].region_info.bounding_box;
      const oneBox = {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch(' https://enigmatic-stream-12568.herokuapp.com/imageurl', {
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch(' https://enigmatic-stream-12568.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
